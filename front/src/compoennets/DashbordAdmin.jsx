...
export default function DashboardAdmin() {
  ...
  const [medicalForm, setMedicalForm] = useState({ childId: '', allergies: '', notes: '' });
  ...
  const handleUserSubmit = (e) => {
    e.preventDefault();
    console.log('Ajouter utilisateur:', userForm);
    alert(`Utilisateur ajouté: ${userForm.name} (${userForm.role})`);
  ...
  return (
   <div className="flex w-screen h-screen bg-gray-100">
      ...
      <aside className="w-64 bg-white shadow-lg p-6 flex flex-col">
        <h2 className="text-2xl font-bold text-pink-500 mb-8 cursor-pointer" onClick={() => setPage('dashboard')}>
          Admin
        </h2>
        <nav className="flex flex-col space-y-4 text-gray-700">
          <button onClick={() => setPage('dashboard')} className={`text-left ${page === 'dashboard' ? 'text-pink-500 font-semibold' : ''}`}>
            🏠 Tableau de bord
          </button>
          <button onClick={() => setPage('addUser')} className={`text-left ${page === 'addUser' ? 'text-pink-500 font-semibold' : ''}`}>
            👤 Ajouter utilisateur
          </button>
          <button onClick={() => setPage('addMedicalFile')} className={`text-left ${page === 'addMedicalFile' ? 'text-pink-500 font-semibold' : ''}`}>
            🩺 Dossier médical
          </button>
        </nav>
      </aside>
  ...
  <div className="flex flex-col items-center justify-center w-full h-full">
    <h1 className="text-3xl font-bold text-pink-500 mb-6">Tableau de bord</h1>
...
{page === 'addUser' && (
  <div className="min-h-screen w-full flex items-center justify-center bg-gradient-to-br from-pink-100 to-white px-4">
    ...
    >
      <h2 className="text-3xl font-semibold text-center text-pink-600 mb-10">Ajouter un utilisateur</h2>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        >
          ...
          <option value="cuisine">Personnel de cuisine</option>
      ...
      >
        ...
        {page === 'addMedicalFile' && (
          <>
            <h2 className="text-2xl font-bold text-pink-500 mb-6">Ajouter un dossier médical</h2>
            <form onSubmit={handleMedicalSubmit} className="space-y-6 bg-white p-6 rounded-xl shadow w-full max-w-xl">
              <input
                name="childId"
                type="text"
              ...
              <textarea
                ...
                rows={4}
              />
              <button type="submit" className="bg-pink-500 text-white px-6 py-3 rounded-xl hover:bg-pink-600 transition">
                Ajouter dossier
              </button>
            </form>
          </>
